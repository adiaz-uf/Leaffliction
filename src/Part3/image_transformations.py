...
class Transform:
    ...
    def remove_background(self):
        ...
        image_blurred = cv2.medianBlur(self.original_img, 5)
        ...
        x = max(0, x - padding)
        y = max(0, y - padding)
        w = min(self.original_img.shape[1] - x, w + padding * 2)
        h = min(self.original_img.shape[0] - y, h + padding * 2)
        rect = (x, y, w, h)

        # Execute GrabCut
        gc_mask = np.zeros(self.original_img.shape[:2], np.uint8)
        ...
        try:
            cv2.grabCut(self.original_img, gc_mask, rect, bgdModel, fgdModel,
        ...
        white_background = np.full(self.original_img.shape, 255,
                                   dtype=np.uint8)
        ...
        result_image[boolean_mask] = self.original_img[boolean_mask]
...
def remove_bg(image_path):
    ...
    blank_background = transform.remove_background()
