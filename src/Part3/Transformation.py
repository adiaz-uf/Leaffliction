...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        usage="%(prog)s [image_path] | [-src SRC_DIR -dst DST_DIR] [transformation_flag] [-h]"
    )
    ...
    parser.add_argument(
        "-src",
        dest="src_dir",
        help="Source directory. Triggers directory mode (requires -dst and a flag)."
    )
    parser.add_argument(
        "-dst",
        dest="dst_dir",
        help="Destination directory. Triggers directory mode (requires -src and a flag)."
    )
    ...
    # Mode 2: Single image processing
        ...
        if args.src_dir or args.dst_dir:
            print("Error: Cannot mix single image path with directory arguments (-src, -dst).", file=sys.stderr)
            parser.print_help()
            sys.exit(1)
        ...
        if file_ext not in valid_extensions:
            print(f"Error: '{args.image_path}' is not a valid image file.", file=sys.stderr)
