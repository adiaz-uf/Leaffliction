...
def split_dataset_stratified(base_dir=".", dst_dir="data", train_ratio=0.8):
    ...
    print(f"  Ratio: {train_ratio*100}% train / {(1-train_ratio)*100}% validation")
    ...
    if output_path.exists():
        ...
        try:
            ...
            if sys.stdin.isatty():
                user_input = input("  Do you want to delete it and start over? (y/n): ").lower()
                if user_input != 'y':
            ...
            print(f"[Error] Could not remove directory '{output_path}': {e}", file=sys.stderr)
            return
    ...
    if not plant_types:
        print(f"[Error] No 'Apple' or 'Grape' directories found in '{base_path}'", file=sys.stderr)
        return
    ...
    for plant_dir in plant_types:
        ...
        for class_path in class_dirs:
            ...
            (train_path / plant_name / class_name).mkdir(parents=True, exist_ok=True)
            (val_path / plant_name / class_name).mkdir(parents=True, exist_ok=True)
            ...
            try:
                ...
                print(f"    [Warning] Could not read files from {class_path}: {e}", file=sys.stderr)
                continue
            ...
            print(f"    - Copying {len(train_images)} images to 'train/{plant_name}/{class_name}'...")
            print(f"    - Copying {len(val_images)} images to 'validation/{plant_name}/{class_name}'...")
            ...
            for img_path in train_images:
                try:
                    ...
                    print(f"    [Error] Failed to copy {img_path}: {e}", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Split dataset into train and validation sets while maintaining directory structure.",
        formatter_class=argparse.RawTextHelpFormatter
    )
    
    parser.add_argument(
        "source_dir",
        help="Source directory containing Apple/ and Grape/ folders with class subdirectories"
    )
    ...
    parser.add_argument(
        ...
        dest="dst_dir",
        default="data_split",
        help="Output directory for train/validation split (default: data_split)"
    )
