...
def load_class_indices(json_path='class_indices.json'):
    ...
    if not os.path.exists(json_path):
        print(
            f"Error: Class indices file '{json_path}'"
...
def display_prediction_result(image_path, predicted_class, confidence):
    ...
    try:
        ...
        if bg_removed_img is not None:
            bg_removed_img_rgb = cv2.cvtColor(
                bg_removed_img, cv2.COLOR_BGR2RGB)
        else:
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Error! Usage is: ./predict.py <image_path>"
            " [model_path] [class_indices_path]")
        print("\nExample:")
        print("  ./predict.py path/to/leaf_image.jpg")
        print("  ./predict.py"
              " path/to/leaf_image.jpg my_model.h5 my_classes.json")
        sys.exit(1)

    image_path = sys.argv[1]
    model_path = sys.argv[2] if len(sys.argv) > 2 else 'leaf_disease_model.h5'
    class_indices_path = sys.argv[3] if len(
        sys.argv) > 3 else 'class_indices.json'
    ...
    if file_ext not in valid_extensions:
        print(
            f"Error: '{image_path}'"
            " is not a valid image file.", file=sys.stderr)
        print(
            f"Supported formats:"
            f" {', '.join(valid_extensions)}", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        predictions = predict_disease(
            model, image_path, class_mapping, top_k=3)
    except Exception as e:
    ...
    if len(predictions) > 1:
        ...
        for i, (class_name, confidence) in enumerate(predictions[1:], 2):
            print(
                f"   {i}. {format_class_name(class_name)}: {confidence:.2f}%")
    ...
    if top_confidence >= 90:
        ...
        print("\nLow confidence - "
              "Consider retaking the image or consulting an expert\n")
